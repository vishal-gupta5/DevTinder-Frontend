...
const Requests = () => {
  ...
  if (requests?.length === 0) return <h1>No Requrest found!</h1>;

  return (
    <div className="flex justify-center flex-col gap-5">
      <h1 className="text-center text-2xl font-bold mt-5">Connection Requests</h1>
      {requests.map((request) => {
        const { firstName, lastName, age, gender, photoURL, about } =
          request.fromUserId;
        return (
          <div key={request._id} className="w-3/7 bg-base-300 mx-auto rounded-lg">
            <div className="flex justify-between gap-5">
              {/* Image  */}
              <div className="flex gap-3">
                <img className="w-32 rounded-full pl-2 p-2" src={photoURL} alt="photo" />
                {/* Inforamtion  */}
                <div className="py-2">
